...
- 재시도 실패: 최소 요약 응답 + `error.code=SCHEMA_VALIDATION_FAILED`
- 리서치 실패: `research.status=fallback`으로 저장 후 전략은 계속 진행
- 보이스 실패: 스크립트 텍스트는 반환하고 TTS는 비동기로 재시도
- 미디어 실패: 프롬프트/카피/보이스 결과는 반환하고 `asset.ready`를 지연 처리
