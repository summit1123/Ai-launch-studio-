...
1. `CHAT_COLLECTING`
- 입력: 사용자 자유 텍스트 대화
- Planner 책임: LLM 추출 + 규칙 추출을 병합해 슬롯 업데이트, 다음 질문 1개, 누락 슬롯 반환
- 사용자 질문이 들어오면 먼저 짧게 답변한 뒤 다음 질문 1개만 이어간다.
- 질문형 문장은 슬롯값으로 오인식하지 않는다.
- 사용자가 "알아서/추천"처럼 결정을 위임하면 현재 브리프 기준 추천값을 제시·적용하고 다음 단계로 진행한다.
...
- 입력이 상충될 때(예: 초특가 + 고가 금액)는 슬롯 확정을 보류하고 검증 질문을 우선한다.
...
- `poster_specs` 최소 1개.
- `video_prompt_package.duration_sec`는 `4|8|12` 중 하나.
- `video_prompt_package.size`는 `480x480|480x854|854x480|720x720|720x1280|1280x720` 중 하나.
- 나레이션 길이 예산: `4초<=28자/1문장`, `8초<=56자/2문장`, `12초<=84자/3문장`.
