# 문서 맵

## 제품
- `mvp.md`: MVP 범위/완료 기준
- `voice_chat_mvp_spec.md`: 음성 대화형(스무고개) 명세
- `business_model.md`: BM 가설/요금/수익 구조
- `expansion.md`: MVP 이후 확장 로드맵

## 아키텍처
- `frontend_architecture.md`: 프론트 구성과 대화 UX
- `backend_architecture.md`: 백엔드 모듈/런타임 설계
- `orchestrator_design.md`: 상태 머신과 에이전트 흐름
- `voice_agent.md`: 보이스 에이전트 역할/출력 계약

## 데이터/API
- `api.md`: 현재 API + 목표 API(대화/스트리밍/음성)
- `db_schema.md`: 현재 SQLite + 목표 스키마

## OpenAI 스택
- `openai_stack.md`: 기능별 OpenAI 활용 계획
- `sora_video_optimization.md`: Sora/음성 품질 최적화

## 개발환경
- `uv_setup.md`: 팀 온보딩(uv 기반)

## 운영/협업
- `codex_workflow.md`: Codex 작업 표준 워크플로
- `skills_guide.md`: 스킬 선택/운영 가이드
